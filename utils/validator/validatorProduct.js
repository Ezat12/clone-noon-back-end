...
const validatorCreateProduct = [
  ...
  check("description")
    .notEmpty()
    .withMessage("description product nust be required")
    .isLength({ min: 10 })
  ...
  check("price_discount")
    ...
    .custom((value, { req }) => {
      if (req.body.price < value) {
        throw new Error("price after discount nust be lower than price");
      }
      return true;
    }),
  ...
  check("subCategory")
    ...
    .custom(async (subcatrgories, { req }) => {
      ...
      const checker = subcatrgories.every((s) => subCategoryAllIds.includes(s));
      if (!checker) {
        throw new Error(
  ...
  check("rating_average")
    ...
    .isLength({ min: 1 })
    .withMessage("rating nust be above or equal 1.0")
    .isLength({ max: 5 })
    .withMessage("rating nust be below or equal 5.0"),
